...
class FrameworkController extends Controller {
  ...
  async jsondbOperation(args) {
    ...
    switch (action) {
      ...
      case "del":
        data.result =
          await Services.get("database.jsondb").delTestData(delete_name);
        break;
      case "update":
        data.result = await Services.get("database.jsondb").updateTestData(
          configData,
      ...
      case "get":
        data.result =
          await Services.get("database.jsondb").getTestData(search_age);
        break;
      case "getDataDir":
      ...
      case "setDataDir":
        data.result =
          await Services.get("database.jsondb").setCustomDataDir(data_dir);
        break;
    }
  ...
  async sqlitedbOperation(args) {
    ...
    switch (action) {
      case "add":
        data.result =
          await Services.get("database.sqlitedb").addTestDataSqlite(info);
        break;
      case "del":
        data.result =
          await Services.get("database.sqlitedb").delTestDataSqlite(
            delete_name
          );
        break;
      case "update":
        data.result = await Services.get(
          "database.sqlitedb"
        ).updateTestDataSqlite(update_name, update_age);
        break;
      case "get":
        data.result =
          await Services.get("database.sqlitedb").getTestDataSqlite(search_age);
        break;
      case "getDataDir":
      ...
      case "setDataDir":
        data.result =
          await Services.get("database.sqlitedb").setCustomDataDir(data_dir);
        break;
    }

    data.all_list =
      await Services.get("database.sqlitedb").getAllTestDataSqlite();
  ...
  async ipcSendMsg(args, event) {
    ...
    const data = await Services.get("framework").bothWayMessage(
      type,
      content,
      event
    );
  ...
  async uploadFile() {
    ...
    const uploadRes =
      await Services.get("framework").uploadFileToSMMS(fileStream);
  ...
  // 文件检索任务
  fileSearchJob(args, event) {
    let jobId = args.id;
    let action = args.action;
    let dirPath = args.dirPath;
    console.log(` 进入 controller - fileseasrchjob`);
    let result;
    switch (action) {
      case "create":
        result = Services.get("framework").doFileSearchJob(
          jobId,
          action,
          dirPath,
          event
        ...
        break;
      case "close":
        Services.get("framework").doFileSearchJob(
          jobId,
          action,
          dirPath,
          event
  ...
  // 翻译任务
  translateJob(args, event) {
    let jobId = args.id;
    let action = args.action;
    let data = JSON.parse(JSON.stringify(args.data));
    console.log(` 进入 controller - translate job`, args);
    let result;
    switch (action) {
      case "create":
        result = Services.get("framework").doTranslateJob(
          jobId,
          action,
          data,
          event
        ...
        break;
      case "close":
        Services.get("framework").doTranslateJob(jobId, action, dirPath, event);
        break;
      default:
    }
